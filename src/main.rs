...
const ROOT_PATH: &str = "/Users/grace.kwak/github/FlightSystems/gnc/gnc_julia_bindings/src/";
...
fn handle_init_or_close(func_name: &str) -> std::io::Result<()> {
    ...
    let outgoing_pipe_path = format!("{}r2j_{}", ROOT_PATH, func_name);
    let incoming_pipe_path = format!("{}j2r_{}", ROOT_PATH, func_name);
    ...
    let mut buffer = [0; NUM_BYTES]; // The command is NUM_BYTES bytes long
    incoming_file.read_exact(&mut buffer)?; // Use `?` to propagate errors
    println!("RUST: Received number: {}", f64::from_le_bytes(buffer));

    let mut result = [0; NUM_BYTES]; // The result is NUM_BYTES bytes long
    if func_name == "init" {
...
fn handle_step() -> io::Result<()> {
    let incoming_pipe_path = format!("{}j2r_step", ROOT_PATH);
    let outgoing_pipe_path = format!("{}r2j_step", ROOT_PATH);
    ...
    let mut buffer = [0; NUM_BYTES]; // Assuming we're working with single-byte commands for simplicity
...
fn rust_step(bytes: [u8; NUM_BYTES]) -> [u8; 8] {
    let slice = &bytes[0..8]; // This is a slice of the first 8 bytes

    let num = f64::from_le_bytes(slice.try_into().expect("slice with incorrect length")); // Convert the bytes to a float
    let val = num * 100.0;
    // sleep(Duration::from_secs(5));
    let bytes = val.to_le_bytes(); // convert the float to little-endian bytes
